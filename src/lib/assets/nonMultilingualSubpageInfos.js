/** @type {{ [key: string]: { projectURL: string; tech: string; year: string; publication: string; } }} */
export const infosUnilingual = {
	vehicle: {
		projectURL: 'https://www.meltt.net/',
		tech: 'HTML, CSS, JavaScript, D3.js, R',
		year: '2021',
		publication:
			'B. Mayer, K. Lawonn, K. Donnay, B. Preim, and M. Meuschke. “VEHICLE: Validation and Exploration of the Hierarchical Integration of Conflict Event Data.” In: <span class="italics">Computer Graphics Forum</span> 40.3 (2021), pp. 1–12. <a class="publication-doi" href="https://doi.org/10.1111/cgf.14284" target="_blank">doi: 10.1111/cgf.14284</a>.'
	},
	methane: {
		projectURL: 'https://experiment-methane-story.vercel.app/',
		tech: 'HTML, CSS, JavaScript, Svelte, GSAP, D3.js',
		year: '2024',
		publication: ''
	},
	characterization: {
		projectURL: 'https://observablehq.com/@zykel/characterization',
		tech: 'HTML, CSS, JavaScript, D3.js, Observable Notebooks',
		year: '2023',
		publication:
			'B. Mayer, N. Steinhauer, B. Preim, and M. Meuschke. “A Characterization of Interactive Visual Data Stories With a Spatio-Temporal Context.” In: <span class="italics">Computer Graphics Forum</span> 42.6 (2023), e14922. <a class="publication-doi" href="https://doi.org/10.1111/cgf.14922" target="_blank">doi: 10.1111/cgf.14922</a>.'
	},
	endovis: {
		projectURL: 'https://isgwww.cs.ovgu.de/~benedikt/endovis_workflow/',
		tech: 'HTML, CSS, JavaScript, D3.js',
		year: '2021',
		publication:
			'B. Mayer, M. Meuschke, J. Chen, B. P. Müller-Stich, M. Wagner, B. Preim, and S. Engelhardt. “Interactive visual exploration of surgical process data.” In: <span class="italics">International Journal of Computer Assisted Radiology and Surgery</span> 18.1 (2023), pp. 127–137. <a class="publication-doi" href="https://doi.org/10.1007/s11548-022-02758-1" target="_blank">doi: 10.1007/s11548-022-02758-1</a>.'
	},
	climate_flows: {
		projectURL:
			'https://www.climatepolicyinitiative.org/resources/data-visualizations/global-landscape-of-climate-finance-data-dashboard/',
		tech: 'HTML, CSS, JavaScript, D3.js',
		year: '2025',
		publication: ''
	},
	exex: {
		projectURL: 'https://matchedwake.com/aid_ex/',
		tech: 'HTML, CSS, JavaScript, React.js, D3.js, R',
		year: '2024',
		publication:
			'B. Mayer, K. Donnay, K. Lawonn, B. Preim, and M. Meuschke. “Expert exploranation for communicating scientific methods - A case study in conflict research.” In: <span class="italics">Computers & Graphics</span> (2024). <a class="publication-doi" href="https://doi.org/10.1016/j.cag.2024.103937" target="_blank">doi: 10.1016/j.cag.2024.103937</a>'
	}
};
